...
class ListCaja extends ListRecords
{
    ...
    public function getActiveCajaProperty(): ?SessioneCaja
    {
    ...
    protected function getAbrirCajaAction(): Action
    {
        return Action::make('abrirCaja')
            ...
            ->action(function (array $data): void {
                ...
                $comment = trim($data['observaciones'] ?? '');
                $formattedObservations = $comment !== '' ? "APERTURA: {$comment}" : null;
    ...
    protected function getCerrarCajaAction(): Action
    {
        return Action::make('cerrarCaja')
            ...
            ->color('danger')
            ->visible(fn (): bool => 
                (Auth::user()?->can('cajas.cerrar') ?? false) &&
                (bool) $this->getActiveCaja()
            )
            ...
            ->form([
                \Filament\Forms\Components\Placeholder::make('saldo_esperado_card')
                    ->label('')
                    ->content(function () {
                        $caja = $this->getActiveCaja();
                        if (!$caja) {
                            return '';
                        }
                        $inicial = $caja->saldo_inicial;
                        $teorico = app(CajaService::class)->saldoTeorico($caja);
                            ...
                            return new \Illuminate\Support\HtmlString("
                                <div class='p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 dark:bg-amber-950/20 dark:border-amber-900/30 flex items-start gap-4 transition-all duration-300 shadow-sm'>
                                    ...
                                    <div class='space-y-0.5 text-left'>
                                        ...
                                        <p class='text-[10px] text-amber-655 dark:text-amber-550 pt-1 leading-normal'>El dinero real supera el saldo esperado. Por favor, especifique el motivo en las observaciones.</p>
                                    </div>
                                </div>
                            ");
                        } elseif ($diff < 0) {
            ...
            ->action(function (array $data): void {
                ...
                $apertura = $caja->getObservacionApertura();
