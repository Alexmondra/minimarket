<div class="kpi-strip">
    ...
    <div class="mb-2 flex items-center justify-between gap-3 px-1">
        <span class="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500">
    ...
    <div class="kpi-strip-grid">
        ...
        @foreach($kpis as $key => $kpi)
            ...
            @endphp
            <div class="kpi-card kpi-strip-card kpi-{{ $kpi['color'] }}">
                <div class="flex items-start justify-between gap-2 mb-2">
                    {{-- Premium Squircle Icon Container --}}
                    <div class="flex h-8 w-8 items-center justify-center rounded-xl bg-{{ $kpi['color'] }}-500/10 text-{{ $kpi['color'] }}-600 dark:text-{{ $kpi['color'] }}-400 border border-{{ $kpi['color'] }}-500/15 transition-colors duration-300">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">{!! $iconSvg !!}</svg>
                    </div>
                    @if($kpi['trend'] !== null)
                        <span @class([
                ...
                {{-- Metric Label --}}
                <p class="mb-0.5 truncate text-[9px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider cursor-default">
                    {{ $kpi['label'] }}
                </p>

                {{-- Metric Value with Smaller/Subtle Currency Prefix --}}
                <p class="flex items-baseline text-base font-extrabold text-slate-900 dark:text-white font-mono tracking-tight cursor-default">
                    @if($kpi['prefix'])
