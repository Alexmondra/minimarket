<div class="space-y-6 py-2">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
            <div class="flex items-center gap-2">
                <span class="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Sucursal</span>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <!-- Saldo Inicial -->
        <div class="p-4 rounded-2xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/30">
            ...
            <p class="text-[9px] text-slate-550 dark:text-slate-400 mt-1 font-semibold">Apertura: {{ $caja->fecha_apertura?->format('d/m/Y H:i') }}</p>
        </div>

        <!-- Ventas Totales -->
        <div class="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/30">
            ...
            <p class="text-[9px] text-slate-550 dark:text-slate-400 mt-1 font-semibold">Solo ventas contadas</p>
        </div>

        <!-- Saldo Teórico -->
        <div class="p-4 rounded-2xl bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-900/30">
            ...
            <p class="text-[9px] text-slate-550 dark:text-slate-400 mt-1 font-semibold">Inicial + Efectivo</p>
        </div>
        ...
        @if(!$caja->estado)
            @php
                ...
                $bgColor = $diff === 0.0 ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/30' : ($diff > 0 ? 'bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/30' : 'bg-rose-50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900/30');
            ...
            @endphp
            <div class="p-4 rounded-2xl border {{ $bgColor }}">
                ...
                <p class="text-[9px] text-slate-550 dark:text-slate-400 mt-1 font-semibold">Real: S/ {{ number_format($caja->saldo_real, 2) }}</p>
            </div>
        @else
            <div class="p-4 rounded-2xl bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                ...
                <p class="text-[9px] text-slate-550 dark:text-slate-400 mt-1 font-semibold font-mono">Cerrar para registrar</p>
            </div>
        @endif
    </div>
    ...
    <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            <!-- Efectivo -->
            <div class="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/60 text-center shadow-sm">
            ...
            <!-- Otro -->
            <div class="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/60 text-center shadow-sm">
                <div class="mx-auto flex h-8 w-8 items-center justify-center rounded-xl bg-slate-500/10 text-slate-500 dark:bg-slate-500/20 dark:text-slate-400">
                    <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
    ...
    <div class="space-y-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <!-- Apertura -->
            <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800 relative shadow-sm">
    ...
    @if($caja->estado && $caja->user_id === auth()->id() && auth()->user()->can('cajas.cerrar'))
        <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end">
            <button 
                ...
                class="inline-flex items-center gap-2 px-5 py-3 bg-rose-600 hover:bg-rose-500 dark:bg-rose-700 dark:hover:bg-rose-600 text-white font-extrabold rounded-2xl shadow-md shadow-rose-600/15 hover:shadow-rose-600/25 transition duration-200 text-xs focus:outline-none focus:ring-2 focus:ring-rose-500 cursor-pointer"
            >
